...
DockablePanel::DockablePanel(Gui *gui,
                             ...
                             QWidget *parent)
    : QFrame(parent), KnobGuiContainerHelper(holder, stack), _imp(new DockablePanelPrivate(this, gui, holder, container, headerMode, useScrollAreasForTabs, helpToolTip))
{
...
bool DockablePanel::isPagingEnabled() const
{
    return _imp->_pagesEnabled;
}

bool DockablePanel::useScrollAreaForTabs() const
{
...
void DockablePanel::onPageLabelChanged(const KnobPageGuiPtr &page)
{
    if (_imp->_tabWidget)
    {
...
void DockablePanel::addPageToPagesContainer(const KnobPageGuiPtr &page)
{
    if (_imp->_tabWidget)
    {
...
void DockablePanel::removePageFromContainer(const KnobPageGuiPtr &page)
{
    if (_imp->_tabWidget)
    {
...
void DockablePanel::setPagesOrder(const std::list<KnobPageGuiPtr> &orderedPages,
...
void DockablePanel::onKnobsRecreated()
{
...
void DockablePanel::onPageIndexChanged(int index)
{
    if (!_imp->_tabWidget)
    {
        return;
...
void DockablePanel::refreshCurrentPage()
{
    onPageIndexChanged(_imp->_tabWidget ? _imp->_tabWidget->currentIndex() : 0);
}

void DockablePanel::onPageActivated(const KnobPageGuiPtr &page)
{
    if (!page)
    {
        return;
...
void DockablePanel::turnOffPages()
{
...
void DockablePanel::setPluginIDAndVersion(const std::string &pluginLabel,
...
void DockablePanel::setPluginIcon(const QPixmap &pix)
{
    if (_imp->_iconLabel)
    {
...
void DockablePanel::onNodeScriptChanged(const QString &label)
{
...
void DockablePanel::onKnobsInitialized()
{
...
void DockablePanel::setPyPlugUIEnabled(bool enabled)
{
    QList<QWidget *> btns;
...
void DockablePanel::refreshTabWidgetMaxHeight()
{
    /*
...
void DockablePanel::refreshUndoRedoButtonsEnabledNess(bool canUndo,
                                                      bool canRedo)
{
...
void DockablePanel::setClosedInternal(bool c)
{
    if (!_imp->_gui)
    {
        return;
...
void DockablePanel::disableUpdate(bool disable)
{
...
void DockablePanel::disableUpdateTimer1()
{
    auto openedPanels = _imp->_gui->getOpenedPanels();
    for (std::list<DockablePanel *>::iterator it = openedPanels.begin(); it != openedPanels.end(); ++it)
    {
...
void DockablePanel::disableUpdateTimer2()
{
...
void DockablePanel::setName(const QString &str)
{
    if (_imp->_nameLabel)
    {
...
void DockablePanel::insertHeaderWidget(int index,
                                       QWidget *widget)
{
...
bool DockablePanel::isMinimized() const
{
...
bool DockablePanel::isClosed() const
{
    QMutexLocker l(&_imp->_isClosedMutex);
...
void DockablePanel::onOverlayColorDialogColorChanged(const QColor &color)
{
    NodeSettingsPanel *nodePanel = dynamic_cast<NodeSettingsPanel *>(this);
...
void DockablePanel::onOverlayButtonClicked()
{
    NodeSettingsPanel *nodePanel = dynamic_cast<NodeSettingsPanel *>(this);
...
bool DockablePanel::hasOverlayColor() const
{
    QMutexLocker locker(&_imp->_currentColorMutex);
...
void DockablePanel::resetHostOverlayColor()
{
    NodeSettingsPanel *nodePanel = dynamic_cast<NodeSettingsPanel *>(this);
...
void DockablePanel::setOverlayColor(const QColor &c)
{
    {
        QMutexLocker locker(&_imp->_currentColorMutex);
        _imp->_overlayColor = c;
...
void DockablePanel::onRightClickMenuRequested(const QPoint &pos)
{
...
void DockablePanel::onManageUserParametersActionTriggered()
{
...
void DockablePanel::setKeyOnAllParameters()
{
    Gui *gui = getGui();
    if (!gui)
    {
...
void DockablePanel::removeAnimationOnAllParameters()
{
    Gui *gui = getGui();
    if (!gui)
    {
...
void DockablePanel::onCenterButtonClicked()
{
    centerOnItem();
}

void DockablePanel::onSubGraphEditionChanged(bool /*editable*/)
{
    // _imp->_enterInGroupButton->setVisible(editable);
}

void DockablePanel::onEnterInGroupClicked()
{
    NodeSettingsPanel *panel = dynamic_cast<NodeSettingsPanel *>(this);
...
void DockablePanel::onHideUnmodifiedButtonClicked(bool checked)
{
    disableUpdate(true);
    if (checked)
    {
...
struct ManageUserParamsDialogPrivate
{
    ...
    ManageUserParamsDialogPrivate(DockablePanel *panel)
        : panel(panel), mainLayout(0), tree(0), items(), buttonsContainer(0), buttonsLayout(0), addButton(0), pickButton(0), editButton(0), removeButton(0), upButton(0), downButton(0), closeButton(0)
    {
    }
