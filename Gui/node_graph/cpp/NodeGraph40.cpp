...
void NodeGraph::togglePreviewsForSelectedNodes()
{
    bool empty = false;
    {
...
void NodeGraph::showSelectedNodeSettingsPanel()
{
...
void NodeGraph::switchInputs1and2ForSelectedNodes()
{
    QMutexLocker l(&_imp->_nodesMutex);
...
void NodeGraph::centerOnItem(QGraphicsItem *item)
{
    _imp->_refreshOverlays = true;
    centerOn(item);
}

void NodeGraph::copyNodes(const NodesGuiList &nodes,
                          NodeClipBoard &clipboard)
{
    _imp->copyNodesInternal(nodes, clipboard);
}

void NodeGraph::copySelectedNodes()
{
...
void NodeGraph::cutSelectedNodes()
{
    if (_imp->_selection.empty())
    {
...
void NodeGraph::pasteCliboard(const NodeClipBoard &clipboard,
...
bool NodeGraph::pasteNodeClipBoards(const QPointF &pos)
{
    QClipboard *clipboard = QApplication::clipboard();
...
bool NodeGraph::pasteNodeClipBoards()
{
    QPointF position = _imp->_root->mapFromScene(mapToScene(mapFromGlobal(QCursor::pos())));
...
void NodeGraph::duplicateSelectedNodes(const QPointF &pos)
{
...
void NodeGraph::duplicateSelectedNodes()
{
...
void NodeGraph::cloneSelectedNodes(const QPointF &scenePos)
{
    if (_imp->_selection.empty())
    {
...
void NodeGraph::cloneSelectedNodes()
{
    QPointF scenePos = _imp->_root->mapFromScene(mapToScene(mapFromGlobal(QCursor::pos())));
...
void NodeGraph::decloneSelectedNodes()
{
    if (_imp->_selection.empty())
    {
...
void NodeGraph::deleteNodePermanantly(const NodeGuiPtr &n)
{
    assert(n);
...
void NodeGraph::invalidateAllNodesParenting()
{
